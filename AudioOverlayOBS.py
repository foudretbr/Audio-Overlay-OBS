...
async def update_media_loop():
    ...
    Explicitly filters for Deezer and Spotify and handles image stream closing to prevent memory leaks.
        ...
        try:
            ...
            if target_session:
                ...
                if info and (info.title or info.artist):
                    ...
                    # Force image check if track changed OR if the cover is missing/failed previously
                    if current_song_id != last_song_id or media_data["cover"] == "":
                        if info.thumbnail:
                            stream = None
                            reader = None
                            try:
                                ...
                                if stream and stream.size > 0:
                                    ...
                                    # Detect actual MIME type using file magic bytes to avoid browser rendering failures
                                    mime_type = "image/jpeg"
                                    if image_bytes.startswith(b'\x89PNG'):
                                        mime_type = "image/png"
                                        
                                    media_data["cover"] = f"data:{mime_type};base64," + base64.b64encode(image_bytes).decode('utf-8')
                                else:
                                    media_data["cover"] = ""
                            except Exception:
                                media_data["cover"] = ""
                            finally:
                                # Crucial: Release WinRT file locks to prevent OS API freezes
                                if reader:
                                    try:
                                        reader.close()
                                    except Exception:
...
def enforce_single_instance():
    ...
    @return: Mutex handle to prevent garbage collection
    """
...
def is_startup_enabled():
    ...
    @return: Boolean indicating startup status
    """
